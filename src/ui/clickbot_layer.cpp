...
bool ClickbotLayer::init(float w, float h, const char* bg, cocos2d::CCRect bgRect) {
...
ClickSettingsLayer* ClickSettingsLayer::create(std::string button, geode::Popup* layer) {
        ...
        if (ret->init(250, 173, Utils::getTexture().c_str())) {
                ret->autorelease();
                return ret;
        }
...
bool ClickSettingsLayer::init(float w, float h, const char* bg, cocos2d::CCRect bgRect) {
        if (!Popup::init(w, h, bg, bgRect)) return false;
        cocos2d::CCPoint offset = (CCDirector::sharedDirector()->getWinSize() - m_mainLayer->getContentSize()) / 2;
    m_mainLayer->setPosition(m_mainLayer->getPosition() - offset);
    m_closeBtn->setPosition(m_closeBtn->getPosition() + offset);
